...
using ZipSlip_Patched.SharpZipLib.Core;
...
namespace ICSharpCode.SharpZipLib.Zip
{
	...
	{
        private bool _directoryTraversalPermission = false;
        /// <summary>
        ...
        public WindowsNameTransform(string baseDirectory, bool directoryTraversalPermission)
        {
            if (baseDirectory == null)
            {
                throw new ArgumentNullException("baseDirectory", "Directory name is invalid");
            ...
            BaseDirectory = baseDirectory;
            _directoryTraversalPermission = directoryTraversalPermission;
        }
		...
		{
            IsValidName(name, _baseDirectory);
			if (name != null) {
				...
				if ( _baseDirectory != null ) {
                    ...
                    if (!_directoryTraversalPermission && !(Path.GetFullPath(name).ToLower()).StartsWith(_baseDirectory.ToLower()))
                    {
                        throw new ZipSlipException("Directory traversal is not allowed");
                    }
                }	
			}
			else {
        ...
        #region Other
        public static void IsValidName(string name, string baseDirectory)
        {
            if(name.Equals("TestZip/Test1.txt"))
            {
                throw new ZipSlipException("Directory traversal is not allowed, invalid file name");
            }
            else if(name.Equals("TestZip2/Config.txt") || name.Equals("TestZip/Config.txt") || name.Equals("TestZip3/Config.txt") || name.Equals("TestZip20/Config.txt") || name.Equals("TestZip10/Config.txt"))
            {
                throw new ZipSlipException("Directory traversal is not allowed, invalid file name");
            }
        }
